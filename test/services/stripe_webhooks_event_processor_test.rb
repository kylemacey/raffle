...
class StripeWebhooksEventProcessorTest < ActiveSupport::TestCase
  setup do
    @event = events(:one)
    @user = users(:one)
  end
  ...
  test "terminal reader setup intent success creates subscription payment tracking" do
    ...
    setup_intent = setup_intent_with_generated_card("seti_webhook_123", order.id, "pm_generated_123")
    event = terminal_event(
      "terminal.reader.action_succeeded",
  ...
  test "terminal reader setup intent success without generated card records failure" do
    order = build_order_with_items([[pos_products(:two), 1]])
    setup_intent = setup_intent_with_generated_card("seti_missing_card", order.id, nil)
    event = terminal_event(
      "terminal.reader.action_succeeded",
      "process_setup_intent",
      "setup_intent",
  ...
  test "terminal reader setup intent success with inactive subscription records failure" do
    order = build_order_with_items([[pos_products(:two), 1]])
    setup_intent = setup_intent_with_generated_card("seti_incomplete_subscription", order.id, "pm_generated_123")
    event = terminal_event(
      "terminal.reader.action_succeeded",
      "process_setup_intent",
      "setup_intent",
  ...
  test "invoice paid event syncs invoice record and creates order" do
  ...
  test "duplicate invoice paid events do not duplicate order" do
    invoice_record = silent_auction_items(:closed_item).create_invoice_record!(
  ...
  test "invoice payment failed event stores failure state" do
    invoice_record = silent_auction_items(:closed_item).create_invoice_record!(
  ...
  test "invoice finalization failed event stores failure state" do
    invoice_record = silent_auction_items(:closed_item).create_invoice_record!(
      stripe_invoice_id: "in_finalization_failed_123",
      stripe_status: "draft",
      amount_cents: 7500,
      customer_name: "Winner",
      customer_email: "winner@example.com"
    ...
    event = invoice_event(
      "invoice.finalization_failed",
      {
        ...
        "attempt_count" => 0,
        "last_finalization_error" => { "message" => "Missing invoice field." }
      }
    )
    ...
    invoice_record.reload
    assert_equal "draft", invoice_record.stripe_status
    assert invoice_record.failed_at
    assert_equal "Missing invoice field.", invoice_record.last_error
  end
  ...
  def setup_intent_with_generated_card(id, order_id, generated_card)
    OpenStruct.new(
      id: id,
      ...
      latest_attempt: OpenStruct.new(
        payment_method_details: OpenStruct.new(
          card_present: OpenStruct.new(generated_card: generated_card)
        )
      )
    )
  end
